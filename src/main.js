...
window.draw = () => {
  ...
  if (selectedColor) {
    if (colorsAt[currentLayer]) { selectedColor = colorsAt[currentLayer] }
  } else {
    selectedColor = colorsAt[Object.keys(colorsAt)[0]]
  }

  for (i = 0; i < imgData.length; i += 4) {
    if (imgData[i + 3] == 0) continue; // skip transparent pixels on png files
...
function colorLayer(currentColor) {
  let result = buildAndMixColors(origColor, currentColor)
